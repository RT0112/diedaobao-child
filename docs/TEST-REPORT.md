...
### 13.1 坐标转换算法完全错误（🔴 严重）
- **文件**：`MapActivity.kt` 第208-212行（JS的 `_tLat/_tLng` 函数）
- **问题**：GCJ-02 加密算法系数完全是乱写的（`2, 3, 0.2, 20, 100...`），不是标准算法
- **影响**：地图上老人位置偏移几百米到几公里，围栏圆心完全不准
- **验证方法**：在深圳拿真实GPS坐标（WGS-84）输入，看地图显示位置是否偏移
- **修复**：替换为标准 GCJ-02 加密/解密算法

### 13.2 添加围栏默认圆心是北京天安门（🔴 严重）
- **文件**：`MapActivity.kt` 第513-514行
- **代码**：`val lat = loc?.latitude ?: 39.9042`（`39.9042, 116.4074` 是天安门坐标）
- **场景**：老人没上报过位置 → 子女添加围栏 → 地图默认显示在**北京**
- **修复**：用子女手机当前位置作为默认圆心，或提示"请先获取老人位置"

### 13.3 围栏半径前后端限制不一致
- **前端**：`MapActivity.kt` 第44行 `radius.coerceIn(50, 2000)`
- **后端**：`server.js` 第417行 `if (r < 50 || r > 5000)`
- **问题**：前端限制2000，后端限制5000，前端改到2000以上会服务端报错

### 13.4 JS桥接保存按钮无防重复点击
- **文件**：`MapActivity.kt` 第41-60行 `onSaveFence()`
- **场景**：用户快速双击保存 → 创建两个同名围栏
- **修复**：加 `isSaving` 标志位

---

### 13.5 WS事件被collect两次导致重复存DB（🟡 重要）
- **问题**：`FamilyGuardianApp.kt` 第85行和 `HomeFragment.kt` 第456行都 collect `WSClient.events`
- **影响**：`FallEvent` 被存**两次**到本地DB，通知页看到重复通知
- **修复**：只在 `FamilyGuardianApp` 里全局存DB，`HomeFragment` 只更新UI不存DB

### 13.6 `fences` List无并发保护（🟡 重要）
- **文件**：`GeofenceFragment.kt` 第99行 `fences.clear()` + 第100行 `fences.addAll()`
- **场景**：用户快速下拉刷新 + 删除围栏 → 两个协程同时修改 `fences`
- **影响**：`ConcurrentModificationException` 崩溃
- **修复**：用 `Mutex` 保护或只用单个协程

### 13.7 `elderId` 多线程读写无保护（🟡 重要）
- **文件**：`CloudBaseClient.kt` 第51行 `var elderId: String? = null`
- **问题**：`bindElder()` 在IO线程写，`getElderId()` 在UI线程读
- **修复**：加 `@Volatile` 或用 `AtomicReference`

---

### 13.8 跌倒报告无本地缓存（🔴 严重）
- **文件**：`FallDetectionService.kt` 第149-197行
- **问题**：网络断开时 `fall-report` 失败，跌倒事件**永久丢失**
- **修复**：本地Room DB缓存失败事件，网络恢复后重试

### 13.9 WS断开后无HTTP降级（🟡 重要）
- **问题**：`WSClient` 断开后，`FallEvent`/`geofence_breach` 推送丢失
- **影响**：子女端可能延迟很久才收到通知（等下次打开App拉取）
- **修复**：`FamilyGuardianApp` 里检测WS断开后，启动HTTP轮询兜底

### 13.10 同一账号多设备互相踢下线（🟡 重要）
- **文件**：`ws.js` 第162-167行
- **代码**：`if (existing && existing !== ws) { existing.close(4001) }`
- **场景**：家里两个子女用同一账号登录 → 互相踢下线
- **修复**：允许同一账号多个WS连接，推送时遍历所有连接

---

### 13.11 传感器50Hz耗电高（🟢 一般）
- **文件**：`SensorCollector.kt`（需确认采样率）
- **问题**：50Hz加速度 + 50Hz陀螺仪 = 常驻高功耗
- **建议**：降至20Hz，或根据充电状态动态调整

### 13.12 位置轮询10秒一次耗电高（🟢 一般）
- **文件**：`FallDetectionService.kt` 第506行 `val interval = if (isUrgentPull) 3_000L else 10_000L`
- **建议**：常态改为30秒，加急时才3秒

### 13.13 远程协助帧100ms轮询流量高（🟢 一般）
- **文件**：`RemoteAssistManager.kt` 第35行 `FRAME_POLL_MS = 100L`
- **影响**：协助10分钟 = 流量可能几百MB
- **建议**：用WS推送帧，避免HTTP轮询

---

### 13.14 Doze模式可能限制后台服务（🟢 一般）
- **问题**：Android 6+ Doze模式会限制后台服务和网络
- **影响**：老人手机锁屏久了，跌倒检测可能停止上报
- **修复**：申请 `REQUEST_IGNORE_BATTERY_OPTIMIZATIONS` 权限

### 13.15 后台启动Activity可能失败（🟢 一般）
- **问题**：Android 10+ 后台启动Activity受限
- **影响**：协助请求在老人锁屏时可能弹不出来
- **修复**：用全屏通知（`fullScreenIntent`）+ 震动 + 响铃多重提醒
...
### P0（立即修复，生产环境不可用）
1. **坐标转换算法错误** → 地图完全不可用
2. **添加围栏默认圆心是天安门** → 新用户完全不可用
3. **跌倒报告无本地缓存** → 网络抖动导致事件丢失
4. **WS事件重复collect** → 重复通知
5. **密码Base64编码** → 安全漏洞
6. **接口无身份验证** → 安全漏洞
